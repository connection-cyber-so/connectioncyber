...
test('coleta carrega System.Security e usa DPAPI sem depender do modulo PowerShell.Security',()=>{assert.match(collect,/Add-Type -AssemblyName System\.Security/);assert.match(validate,/Add-Type -AssemblyName System\.Security/);assert.match(collect,/Read-Host \$Prompt -AsSecureString/);assert.match(collect,/ProtectedData\]::Protect/);assert.match(validate,/ProtectedData\]::Unprotect/);assert.doesNotMatch(collect,/ConvertFrom-SecureString/)});
