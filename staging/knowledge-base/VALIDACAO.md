# M22 — Biblioteca Técnica — Evidências 0.1.1
...
Ainda não houve teste de navegador, Storage HTTP, IA externa, CI remoto ou Preview Vercel nesta etapa. Não houve alteração de pagamentos ou deploy. A validação remota atual cobre schema, policies, Auth, MFA, membership, papel e entitlement; não deve ser descrita como homologação completa do portal.
...
- Executar a jornada M0–M4 e Storage HTTP pelo Preview com dois curadores independentes.
...
Resultado: implementação local e fronteira de autorização homologadas no Supabase staging; portal/Storage/IA ainda pendentes. Não declarar M4 de produção concluído.
