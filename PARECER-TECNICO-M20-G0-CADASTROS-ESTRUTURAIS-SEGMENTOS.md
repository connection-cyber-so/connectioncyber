...
| M20-G3 | ✅ Concluído (07/09/2026). Painéis expansíveis (`<details>`, sem JS de navegação) com Documentos/Contatos/Endereços por pessoa e Fiscal/Comercial/Atributos por item — exigiu bump do contrato de comandos travado do M18 (`M18-VISUAL-1.0`→`M20-VISUAL-2.0`, 7→13 comandos). Ver `RELATORIO-M20-G3-TELAS-EM-ABAS.md` | Feito | — |
| M20-G4 | ✅ Concluído (07/09/2026). Seletor real em `/vendas`/`/servicos` (filtra Supabase de verdade) e em `/operacoes`/`/catalogo` (estoque segue consolidado por decisão registrada — refatorar quebraria 7 testes existentes sem ganho real). Ver `RELATORIO-M20-G4-SELETOR-ESTABELECIMENTO.md` | Feito | — |
