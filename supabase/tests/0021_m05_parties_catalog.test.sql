...
select has_table('public',t) from unnest(array[
...
select hasnt_table_privilege('anon','public.erp_parties','SELECT','anon não lê pessoas');
select hasnt_table_privilege('anon','public.erp_catalog_items','SELECT','anon não lê catálogo ERP');
select has_table_privilege('authenticated','public.erp_parties','SELECT','authenticated possui grant filtrado por RLS');
select has_table_privilege('authenticated','public.erp_catalog_items','SELECT','authenticated possui grant filtrado por RLS');
select hasnt_table_privilege('authenticated','public.erp_parties','DELETE','cadastro usa exclusão lógica');
select hasnt_table_privilege('authenticated','public.erp_catalog_items','DELETE','catálogo usa exclusão lógica');
