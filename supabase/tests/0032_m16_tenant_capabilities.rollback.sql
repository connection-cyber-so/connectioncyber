...
('16000000-0000-4000-8000-000000000001','finance','deny',repeat('d',64),'approval:sha256:'||repeat('1',64),'2026-01-01T00:00:00Z','2027-01-01T00:00:00Z'),
('16000000-0000-4000-8000-000000000001','inventory','allow',repeat('e',64),'approval:sha256:'||repeat('2',64),'2026-01-01T00:00:00Z','2027-01-01T00:00:00Z');
...
select throws_ok($$insert into public.erp_tenant_capability_exceptions(tenant_id,capability_key,effect,reason_hash,approval_ref,effective_from,expires_at)values('16000000-0000-4000-8000-000000000001','cash','deny',repeat('f',64),'approval:sha256:'||repeat('4',64),now(),now())$$,'23514',null,'expiracao invalida recusada');
...
select ok(not has_table_privilege('service_role','public.erp_tenant_capability_entitlements','UPDATE'),'service role nao muta entitlement');
select ok(not has_table_privilege('service_role','public.erp_tenant_capability_exceptions','DELETE'),'service role nao exclui excecao');
...
select ok(has_function_privilege('service_role','public.erp_revoke_capability_exception(uuid,uuid,timestamp with time zone)','EXECUTE'),'service role executa revogador');
select ok(not has_function_privilege('authenticated','public.erp_revoke_capability_exception(uuid,uuid,timestamp with time zone)','EXECUTE'),'authenticated nao revoga excecao');
set local role service_role;
select set_config('request.jwt.claim.role','service_role',true);
select is((select status from public.erp_resolve_tenant_capabilities('16000000-0000-4000-8000-000000000001','2026-08-30T12:00:00Z')where capability_key='finance'),'disabled','deny prevalece no resolver');
select is((select status from public.erp_resolve_tenant_capabilities('16000000-0000-4000-8000-000000000001','2026-08-30T12:00:00Z')where capability_key='inventory'),'enabled','allow habilita no resolver');
select is((select status from public.erp_resolve_tenant_capabilities('16000000-0000-4000-8000-000000000001','2026-08-30T12:00:00Z')where capability_key='catalog'),'disabled','ausencia permanece deny');
select is((select public.erp_revoke_capability_exception('16000000-0000-4000-8000-000000000001',(select id from public.erp_tenant_capability_exceptions where capability_key='inventory'),now())),true,'revogacao controlada funciona');
select is((select public.erp_revoke_capability_exception('16000000-0000-4000-8000-000000000001',(select id from public.erp_tenant_capability_exceptions where capability_key='inventory'),now())),false,'revogacao e idempotente');
select is((select status from public.erp_resolve_tenant_capabilities('16000000-0000-4000-8000-000000000001','2026-08-30T12:00:00Z')where capability_key='inventory'),'disabled','excecao revogada deixa de habilitar');
reset role;
