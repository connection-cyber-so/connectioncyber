...
begin
  ...
  if v_schema is not null and v_schema<>'extensions'then raise exception 'pgvector instalado em schema divergente: %',v_schema;end if;
  if exists(select 1 from information_schema.columns where table_schema='public'and table_name='erp_catalog_items'and column_name in('embedding','fts'))then raise exception 'Objetos CTR já existem sem histórico 0027';end if;
end $$;
