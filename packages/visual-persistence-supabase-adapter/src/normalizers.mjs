...
// M20-G3/G4: erp_business_verticals/erp_vertical_attribute_requirements são catálogo
// global (sem coluna tenant_id, mesmo padrão de erp_segment_profiles/erp_permissions) —
// tenantRows() rejeitaria toda linha com ACCESS_DENIED por falta de tenant_id, então
// pulam a filtragem por tenant deliberadamente, nunca por omissão.
const GLOBAL_READ_MODELS=new Set(['business-verticals','vertical-attribute-requirements']);
export function normalizeReadModel(key,value,tenantId){
 const source=GLOBAL_READ_MODELS.has(key)?rows(value):tenantRows(value,tenantId);
 ...
 if(key==='party-documents'||key==='party-contacts'||key==='party-addresses'||key==='item-fiscal-data'||key==='item-commercial-data'||key==='establishments')return structuredClone(source);
 if(key==='business-verticals'||key==='vertical-attribute-requirements')return structuredClone(source);
