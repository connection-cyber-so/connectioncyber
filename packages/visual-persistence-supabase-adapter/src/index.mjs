...
export function createSupabasePersistenceTransport({client,aggregateReader}={}){
 ...
 return Object.freeze({
  ...
  async read(contract,tenantId){assertTenant(tenantId);if(contract?.tenantFilter!=='server-resolved')fail('ADAPTER_READ_BLOCKED');if(contract.source==='server-aggregate'){if(typeof aggregateReader!=='function')fail('ADAPTER_NOT_CONFIGURED');return aggregateReader({client,tenantId,scope:contract.screen==='/'?'dashboard':'finance'});}const plan=READ_PLANS[readKey(contract)];if(!plan)fail('ADAPTER_READ_BLOCKED');let query=client.from(plan.table).select(plan.columns).eq('tenant_id',tenantId);if(plan.eq)query=query.eq(...plan.eq);if(plan.in)query=query.in(...plan.in);if(plan.order)query=query.order(...plan.order);if(plan.limit)query=query.limit(plan.limit);const{data,error}=await query;if(error)fail(typeof error.code==='string'?error.code:'PERSISTENCE_FAILURE','persistence read failed');if(!Array.isArray(data))fail('INVALID_PERSISTENCE_RESPONSE');return normalizeReadModel(contract.key,plan.transform==='stock-balance'?stockBalance(data):data,tenantId);}
 });
}
