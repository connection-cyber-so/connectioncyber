...
const item = (sequence = 2, overrides = {}) => syntheticCommand('catalog.item.create', sequence, { marker: 'SYNTHETIC-MASTER-DATA', kind: 'product', code: 'SYNTHETIC-001', name: 'SYNTHETIC PRODUCT', description: 'SYNTHETIC DESCRIPTION', trackInventory: true, allowsFraction: false, priceCents: 1000, ...overrides });
...
test('preço de catálogo não positivo é recusado', async () => await assert.rejects(setup().app.execute({ host, command: item(1, { priceCents: 0 }) }), /INVALID_COMMAND/));
